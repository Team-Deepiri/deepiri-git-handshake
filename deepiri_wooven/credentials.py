...
def setup_https_git(host: str = "github.com") -> list[str]:
    ...
    if cm.git_helper_registered():
        report.append("git credential helper 'wooven' is enabled — HTTPS PATs load from the OS keyring.")
    if cm.get_pat(host):
        report.append(f"PAT for {host}: stored ({cm.pat_status_line(host)}).")
    else:
        report.append(f"No PAT in keyring for {host} yet — use the Vault tab or `wooven cred pat --host`.")
...
def manager_summary(host: str) -> list[str]:
    ...
    lines = [
        ...
        f"  git helper 'wooven': {'yes' if cm.git_helper_registered() else 'no'}",
    ]
    return lines
