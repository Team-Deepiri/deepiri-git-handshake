...
from deepiri_wooven.daemon import daemon_request
from deepiri_wooven.transport_prefs import install_state_path


def _real_git() -> str:
    env_path = os.environ.get("WOOVEN_REAL_GIT", "").strip()
    ...
    if found:
        return found
    raise SystemExit("deepiri-wooven: real git binary not found")
...
def _maybe_rewrite_clone(argv: list[str]) -> list[str]:
    ...
    if url != source:
        print(
            f"deepiri-wooven: using {transport.upper()} → {url}",
            file=sys.stderr,
        )
    new_argv = list(argv)
