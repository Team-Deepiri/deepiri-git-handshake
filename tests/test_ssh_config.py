...
from deepiri_wooven.ssh_config import apply_identity_block, strip_managed_block
...
def test_strip_managed_block_roundtrip(tmp_path: Path, monkeypatch: object) -> None:
    ...
    assert "deepiri-wooven begin github.com" in body
    assert "IdentityFile" in body
    stripped = strip_managed_block(body, "github.com")
    assert "deepiri-wooven" not in stripped
